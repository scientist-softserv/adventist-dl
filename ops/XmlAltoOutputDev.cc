            ...
            if ((rightClass != NOT_A_MODIFIER || leftClass != NOT_A_MODIFIER) && overlap) {
                ...
                if (rightClass != NOT_A_MODIFIER) {
                    if (leftClass == NOT_A_MODIFIER) {
                        ...
                        baseChar = new UnicodeString(static_cast<UChar32>(getStandardBaseChar(chPrev->c)));
                    }
                } else if (leftClass != NOT_A_MODIFIER) {
                    diactritic = getCombiningDiacritic(leftClass);
                    baseChar = new UnicodeString(static_cast<UChar32>(getStandardBaseChar(ch->c)));
                }

                if (diactritic != 0) {
                    diacriticChar = new UnicodeString(static_cast<UChar32>(diactritic));
    ...
    if (overlap && len > 0) {
        ...
        if (leftClass != NOT_A_MODIFIER || rightClass != NOT_A_MODIFIER) {
            ...
            if (leftClass != NOT_A_MODIFIER) {
                if (rightClass == NOT_A_MODIFIER) {
                    diactritic = getCombiningDiacritic(leftClass);
                    baseChar = new UnicodeString(static_cast<UChar32>(getStandardBaseChar(u)));
                }
                ...
                diactritic = getCombiningDiacritic(rightClass);
                baseChar = new UnicodeString(static_cast<UChar32>(getStandardBaseChar(prvChar)));
            }
