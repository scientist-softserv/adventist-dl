...
module Bulkrax
  ...
  class RerunErroredEntriesForImporterService
    ...
    #        :error_classes.
    # @param logger [Logger]
    def self.call(importer_id:, last_run_id: nil, error_classes: [], logger: Rails.logger)
      new(importer_id: importer_id, last_run_id: last_run_id, error_classes: error_classes, logger: logger)
    end
