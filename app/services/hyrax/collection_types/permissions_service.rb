...
module Hyrax
  module CollectionTypes
    class PermissionsService
      ...
      #
      # @param roles [String] type of access, Hyrax::CollectionTypeParticipant::MANAGE_ACCESS
      # and/or Hyrax::CollectionTypeParticipant::CREATE_ACCESS
      ...
      # @note Several checks get the user's groups from the user's ability.  The same values can
      # be retrieved directly from a passed in ability.
      #   If calling from Abilities, pass the ability.  If you try to get the ability from the user,
      # you end up in an infinit loop.
      ...
      # @note Several checks get the user's groups from the user's ability.  The same values can be retrieved directly from a passed in ability.
      #   If calling from Abilities, pass the ability.  If you try to get the ability from the user, you end up in an infinit loop.
      def self.access_to_collection_type?(collection_type:, access:, user: nil, ability: nil)
        return false unless user.present? || ability.present?
