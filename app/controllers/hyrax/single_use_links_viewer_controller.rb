...
module Hyrax
  class SingleUseLinksViewerController < DownloadsController
    ...
    def show
      ...
      # create a dowload link that is single use for the user since we do not just want
      # to show metadata we want to access it too
    ...
    private
      ...
      def search_service
        Hyrax::SearchService.new(config: blacklight_config,
                                 user_params: { id: single_use_link.item_id },
                                 scope: self)
      end
      ...
      def single_use_link
        @single_use_link ||= SingleUseLink.find_by!(download_key: params[:id])
      end
      ...
      def asset
        # @asset ||= Hyrax.query_service.find_by_alternate_identifier(alternate_identifier: params[:id])
        # Line below was line above but was modified to keep Hyrax v2.9.5 prefered method of locating records
        @asset ||= Hyrax.query_service.find_by(id: params[:id])
      end
      ...
      def render_single_use_error(exception)
        logger.error("Rendering PAGE due to exception:
          #{exception.inspect} - #{exception.backtrace if exception.respond_to? :backtrace}")
